...
def load_tasks():
    ...
    if temp_data_get != []:
        for i in range(len(temp_data_get)):
            task_items.append([temp_data_get[i]['id'],temp_data_get[i]['task'],temp_data_get[i]['date'],temp_data_get[i]['complete state']])
        return task_items
    else:
        pass
...
def add_task(task,date):
    ...
    item_data['id'] = int(index_counter)
    item_data['task'] = task
    item_data['date'] = date
...
def pop_task(index):
    ...
    for i in range(len(temp_data_pop)):
        temp_data_pop[i]["id"] = index_counter
        index_counter += 1
...
def get_tasks():
    ...
    if temp_data_get != []:
        for i in range(len(temp_data_get)):
            task_items = ([temp_data_get[i]['id'],temp_data_get[i]['task'],temp_data_get[i]['date'],temp_data_get[i]['complete state']])
        return task_items
    else:
        pass
...
while True:
    ...
    if ch == 1:
        ...
        index = int(input("Give id to delete:"))
        pop_task(index)
    elif ch == 3:
        break
    else:
